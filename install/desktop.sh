...
install_desktop_environment() {
    ...
    if ! command -v proot-distro >/dev/null 2>&1; then
        echo "Error: proot-distro is required."
        return 1
    fi
    ...
    proot-distro login ubuntu -- bash -c "
        ...
        apt-get update -y
        apt-get install -y --no-install-recommends             xfce4 xfce4-terminal xfce4-session xfce4-panel             xfwm4 thunar xfce4-settings xfce4-appfinder             desktop-base light-locker x11-xserver-utils x11-xfs-utils             adwaita-icon-theme fonts-liberation
    "
